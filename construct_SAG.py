...
def generate_semantic_embeddings(dataset_type, news_category, mode):
    ...
    with open('data/dict/vocabulary-0-MIND-small.json', 'r', encoding='utf-8') as f:
        vocab = json.load(f)
    with open('data/dict/word_embedding-0-300-MIND-small.pkl', 'rb') as f1:
        word_embeddings = pickle.load(f1)
    t_s_e, c_s_e = [], []
    if not os.path.exists(title_semantic_embedding_file) or not os.path.exists(content_semantic_embedding_file):
        ...
        for index in tqdm(range(len(titles))):
            titles_word = pat.findall(titles[index].lower().replace('é', 'e'))
            for i in titles_word:
                if i in list(vocab.keys()):
                    t = word_embeddings[vocab[i]]
                    t_e.append(t)
                else:
                    t_e.append(torch.zeros(300))

        for inde in tqdm(range(len(contents))):
            content_word = pat.findall(contents[inde].lower().replace('é', 'e'))
            for i in content_word:
                if i in list(vocab.keys()):
                    c = word_embeddings[vocab[i]]
                    c_e.append(c)
                else:
                    c_e.append(torch.zeros(300))

            t_s_e.append(torch.mean(torch.stack(t_e), dim=0))
            c_s_e.append(torch.mean(torch.stack(c_e), dim=0))

        title_semantic_embeddings = torch.stack(t_s_e)
        content_semantic_embeddings = torch.stack(c_s_e)
...
def generate_cos_similarities(dataset_type, top_M, category, title_semantic_embeddings, content_semantic_embeddings,
                              corpus_title_semantic_embeddings, corpus_content_semantic_embeddings):
    ...
    if not os.path.exists(title_cos_values_file) or not os.path.exists(title_cos_indices_file) or not os.path.exists(
            content_cos_values_file) or not os.path.exists(content_cos_indices_file) or not os.path.exists(
            title_content_cos_values_file) or not os.path.exists(title_content_cos_indices_file) or not os.path.exists(
            content_title_cos_values_file) or not os.path.exists(content_title_cos_indices_file) or not os.path.exists(
            average_cos_values_file) or not os.path.exists(average_cos_indices_file):
        ...
        with torch.no_grad():
            for i in range(news_num):
                title_title_cos = cos(title_semantic_embeddings[i].unsqueeze(dim=0).repeat(corpus_news_num, 1),
                                      corpus_title_semantic_embeddings)
                values, indices = torch.topk(title_title_cos, k=top_M + 1)
                title_cos_values[i] = values
                title_cos_indices[i] = indices
                content_content_cos = cos(content_semantic_embeddings[i].unsqueeze(dim=0).repeat(corpus_news_num, 1),
                                          corpus_content_semantic_embeddings)
                values, indices = torch.topk(content_content_cos, k=top_M + 1)
                content_cos_values[i] = values
                content_cos_indices[i] = indices
                title_content_cos = cos(title_semantic_embeddings[i].unsqueeze(dim=0).repeat(corpus_news_num, 1),
                                        corpus_content_semantic_embeddings)
                ...
                content_title_cos = cos(content_semantic_embeddings[i].unsqueeze(dim=0).repeat(corpus_news_num, 1),
                                        corpus_title_semantic_embeddings)
                ...
                values, indices = torch.topk(
                    (title_title_cos + content_content_cos + title_content_cos + content_title_cos) / 4, k=top_M + 1)
...
def generate_similariy_info(dataset_type, top_M, category, news_dict_inv, corpus_news_dict_inv, title_cos_values,
                            title_cos_indices, content_cos_values, content_cos_indices, title_content_cos_values,
                            title_content_cos_indices, content_title_cos_values, content_title_cos_indices,
                            average_cos_values, average_cos_indices):
    ...
    title_content_similarity_file = dataset_type + '-SAG/similarity/title-content_similarity-%d-%s.json' % (
    top_M, category)
    content_title_similarity_file = dataset_type + '-SAG/similarity/content-title_similarity-%d-%s.json' % (
    top_M, category)
    average_similarity_file = dataset_type + '-SAG/similarity/average_similarity-%d-%s.json' % (top_M, category)
    if not os.path.exists(title_similarity_file) or not os.path.exists(content_similarity_file) or not os.path.exists(
            title_content_similarity_file) or not os.path.exists(content_title_similarity_file) or not os.path.exists(
            average_similarity_file):
        title_similarity = {}
        content_similarity = {}
...
def generate_similar_news_list(dataset_type, top_M, category, news_dict_inv, corpus_news_dict_inv, empty_news_IDs,
                               title_similarity, content_similarity, title_content_similarity, content_title_similarity,
                               average_similarity):
    assert len(news_dict_inv) == len(title_similarity)
    news_num = len(news_dict_inv)
    corpus_news_num = len(corpus_news_dict_inv)
    top_M = min(top_M, corpus_news_num - 1)
    news_title_similarity_file = dataset_type + '-SAG/news_similarity/news_title_similarity-%d-%s.json' % (
    top_M, category)
    news_content_similarity_file = dataset_type + '-SAG/news_similarity/news_content_similarity-%d-%s.json' % (
    top_M, category)
    news_title_content_similarity_file = dataset_type + '-SAG/news_similarity/news_title-content_similarity-%d-%s.json' % (
    top_M, category)
    news_content_title_similarity_file = dataset_type + '-SAG/news_similarity/news_content-title_similarity-%d-%s.json' % (
    top_M, category)
    news_average_similarity_file = dataset_type + '-SAG/news_similarity/news_average_similarity-%d-%s.json' % (
    top_M, category)
    if not os.path.exists(news_title_similarity_file) or not os.path.exists(
            news_content_similarity_file) or not os.path.exists(
            news_title_content_similarity_file) or not os.path.exists(
            news_content_title_similarity_file) or not os.path.exists(news_average_similarity_file):
...
def aggregate(dataset_type, top_M, news_category_dict):
    ...
    if not os.path.exists(similarity_file):
        ...
        with open('data/dict/news_ID-' + dataset_type + '.json', 'r', encoding='utf-8') as f:
...
def visualize(dataset_type, news_ID_dict, train_root, dev_root, test_root, news_node_ID, news_graph, news_node_num,
              top_M, hop, suffix=''):
    news_ID_dict_inv = {news_ID_dict[news_ID]: news_ID for news_ID in news_ID_dict}
    title_dict = {}
...
def construct_SAG(dataset_type, train_root, dev_root, test_root, top_M, hop, news_node_num, news_ID_dict):
    ...
    with open('data/dict/category-' + dataset_type + '.json', 'r', encoding='utf-8') as f:
    ...
    for category in news_category_dict:
        if non_empty_corpus[category]:
            news_dict1, news_dict_inv1, titles1, contents1, empty_news_IDs1, title_semantic_embeddings1, content_semantic_embeddings1 = generate_semantic_embeddings(
                dataset_type, category, mode='full')
            news_dict2, news_dict_inv2, titles2, contents2, empty_news_IDs2, title_semantic_embeddings2, content_semantic_embeddings2 = generate_semantic_embeddings(
                dataset_type, category, mode='corpus')
            title_cos_values, title_cos_indices, content_cos_values, content_cos_indices, title_content_cos_values, title_content_cos_indices, content_title_cos_values, content_title_cos_indices, average_cos_values, average_cos_indices = generate_cos_similarities(
                dataset_type, top_M, category, title_semantic_embeddings1, content_semantic_embeddings1,
                title_semantic_embeddings2, content_semantic_embeddings2)
            title_similarity, content_similarity, title_content_similarity, content_title_similarity, average_similarity = generate_similariy_info(
                dataset_type, top_M, category, news_dict_inv1, news_dict_inv2, title_cos_values, title_cos_indices,
                content_cos_values, content_cos_indices, title_content_cos_values, title_content_cos_indices,
                content_title_cos_values, content_title_cos_indices, average_cos_values, average_cos_indices)
            generate_similar_news_list(dataset_type, top_M, category, news_dict_inv1, news_dict_inv2, empty_news_IDs1,
                                       title_similarity, content_similarity, title_content_similarity,
                                       content_title_similarity, average_similarity)
    news_similarity_dict = aggregate(dataset_type, top_M, news_category_dict)
    assert len(news_similarity_dict) == len(news_ID_dict), str(len(news_similarity_dict)) + ' : ' + str(
        len(news_ID_dict))
    # 语意增强图
    news_node_ID, news_graph, news_graph_mask = generate_news_graph(dataset_type, news_similarity_dict, news_ID_dict,
                                                                    top_M, hop, news_node_num)
    return news_node_ID, news_graph, news_graph_mask
